...
fn test_import_refs_empty_git_repo() {
    ...
    let mut tx = test_data.repo.start_transaction().unwrap();
    git::import_refs(tx.repo_mut(), &git_settings).unwrap();
    tx.repo_mut().rebase_descendants().unwrap();
    let repo = tx.commit("test").unwrap();
...
fn test_fetch_with_fetch_tags_override() {
    ...
    let mut tx = test_repo.repo.start_transaction().unwrap();
    git::add_remote(
        tx.repo_mut(),
        "origin".as_ref(),
...
fn set_up_push_repos(settings: &UserSettings, temp_dir: &TempDir) -> PushTestSetup {
    ...
    let parent_of_main_commit = jj_repo
        ...
        .unwrap();
    let mut tx = jj_repo.start_transaction().unwrap();
...
fn test_push_bookmarks_success() {
    ...
    let mut tx = setup.jj_repo.start_transaction().unwrap();
